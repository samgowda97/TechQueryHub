...
//     e.preventDefault();
//     if (newQuestion.trim() !== '') {
//       dispatch(addQuestion({ text: newQuestion }));
//       setNewQuestion('');
//     }
//   };
...
//     e.preventDefault();
//     dispatch(addAnswer({ questionId: selectedQuestionId, text: answerText }));
//     setAnswerText('');
...
//           <p><span style={{backgroundColor:"red"}}>{question.user}</span> <strong>Question:</strong> {question.text}</p>
//           <ul>
//             {answers.filter((answer) => answer.questionId === question.id).map((answer) => (
//               <div>
//                 <p key={answer.id}><span style={{backgroundColor:"blue"}}>{`${answer.user}`}</span>{answer.text}</p>
...
import { addQuestion } from './questionsSlice';
import { addAnswer } from './answersSlice';
import { loginUser, logoutUser } from './userSlice'; // Import the login/logout actions
  ...
  return (
    <div>
      {isLoggedin ? (
        ...
        <div>
        {/* //   <h2>Login</h2>
        //   <input type="text" value={userr} placeholder="Username" onChange={(e) => setUser(e.target.value)} />
        //   <button onClick={handleLogin(userr)}>Login</button>
        // </div> */}
          <h2>Login</h2>
